...
function OController(options) {
    ...
    OController.prototype.loadFragment = function(file, callback, cache, component) {
    	if (!component)
    ...
    if (this.options.rewriteUrl) {
        ...
        if( pos > -1 ) {
            if( lastPos > -1 ) {
            	this.parameter( 'rid', parent.location.hash.substring(lastPos+2) );
                this.loadFragment( parent.location.hash.substring(pos+1, lastPos) );
            } else
